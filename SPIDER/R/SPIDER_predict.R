...
#' @param all_trainable_proteins_gene_names A data frame matching all trainable proteins' (i.e., seen proteins in your reference set) names to their corresponding gene names. Columns must contain column names: 'consistent_protein_name' specifying the protein names, and 'gene_name' specifying the corresponding gene names.
#' @param file_A_B_C_matching_table A data frame specifying the path to the reference data split by their context factors. Must containing column names: 'file_B' specifying all file paths of embedded reference transcriptomes, 'file_C' specifying all corresponding file paths of reference CLR normalized ADT counts, 'tissue_class' specifying corresponding reference tissue classes using numeric numbers (0, 1, 2, ...), 'disease_class' specifying corresponding reference disease classes using numeric numbers (0, 1, 2, ...), 'cell_type_class' specifying corresponding reference cell type classes using numeric numbers (0, 1, 2, ...). Make sure that each row only contain one cell type, one disease and one tissue correctly describing the states of cells in the corresponding transcriptome and ADT files. 
